...
- Define API contracts in the openspec proposal (`paths`, `schemas`, auth requirements) — TechLead cannot gate-check without them
- Always include `api_contracts` section in openspec proposals for features that cross the frontend/backend/mobile boundary
...
- When a `contract-deviation` is escalated by TechLead: review the deviation report, clarify or amend the openspec contract, then re-announce `spec-ready` with the corrected contract path
