...
Take a PRD (markdown file) from `{{.PRDOutputPath}}` and convert it to YAML task entries in `.agentic/tasks/backlog.yaml`.
...
**Input PRD** (from `{{.PRDOutputPath}}prd-task-status.md`):
```markdown
...
1. **Read the PRD** from `{{.PRDOutputPath}}prd-[feature-name].md`
